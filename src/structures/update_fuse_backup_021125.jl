...
function update_fuse_for_pax!(
    ...
    # NILS: shortest/practical Newton solver for θ in θ - sin θ = K, K = 2S/r^2
    function theta_from_area(r, S; tol=1e-12, maxiter=50)
        S <= 0 && return 0.0                   # trivial
        K = 2S/(r*r)
        θ = (6K)^(1/3)  # very good initial guess for small K
        for i in 1:maxiter
            f = θ - sin(θ) - K
            abs(f) < tol && return θ
            df = 1 - cos(θ)
            if abs(df) < 1e-16
                θ += 1e-6  # dodge near-zero derivative (rare)
            else
                θ -= f/df
            end
        end
    ...
    if fuse.n_decks == 2 #if the aircraft is a double decker
        ...
        if (Vfcs == 0.0 || fcs_loc == 1.0 || run_from_within == false)  # NILS: fcs_loc == 0.0 stands for "underfloor", whereas 1.0 stands for "rear" to comply with numeric slot `parg = zeros(Float64, igtotal)` in read_input.jl
        ...
        elseif (Vfcs > 0.0 && fcs_loc == 0.0 && run_from_within == true)  # NILS: fcs_loc == 0.0 stands for "underfloor", whereas 1.0 stands for "rear" to comply with numeric slot `parg = zeros(Float64, igtotal)` in read_input.jl
            pim2θ = theta_from_area(Rfuse, A_cargo + Vfcs / lcyl)  # NILS
            θ = -(pi - pim2θ) / 2  # NILS (NOTE the "-" sign - look inside theta_from_area())
        end
    ...
    if (Vfcs == 0.0 || fcs_loc == 1.0 || run_from_within == false)  # NILS: fcs_loc == 0.0 stands for "underfloor", whereas 1.0 stands for "rear" to comply with numeric slot `parg = zeros(Float64, igtotal)` in read_input.jl
        ...
        pim2θ = pi - 2 * (-θ)  # NILS (NOTE the "-" sign - look inside theta_from_area())
        A_cargo = fuse.layout.radius^2 / 2 * (pim2θ - sin(pim2θ))  # NILS
        run_from_within = true  # NILS: re-run with updated θ (other conditions apply further down)
    elseif (Vfcs > 0.0 && fcs_loc == 0.0 && run_from_within == true)  # NILS: fcs_loc == 0.0 stands for "underfloor", whereas 1.0 stands for "rear" to comply with numeric slot `parg = zeros(Float64, igtotal)` in read_input.jl
        pim2θ = theta_from_area(fuse.layout.radius, A_cargo + Vfcs / lcyl)  # NILS
        θ = -(pi - pim2θ) / 2  # NILS (NOTE the "-" sign - look inside theta_from_area())
